...
#
# Usage:
#   ./deploy_mafqood2.sh start         Start all services in background
#   ./deploy_mafqood2.sh stop          Stop all services
#   ./deploy_mafqood2.sh restart       Restart all services
#   ./deploy_mafqood2.sh status        Check running status of services
#   ./deploy_mafqood2.sh logs [svc]    Tail logs (django, celery, beat, ngrok, all)
# ====================================================================
...
LOCKFILE="/tmp/mafqood_orchestrator2.lock"
...
# --------------------------------------------------------------------
check_start_redis() {
    echo -e "${BLUE}[*] Verifying Redis connection on port 6390...${NC}"
...
PY
    then
        ...
        echo -e "${YELLOW}[!] Redis is NOT running. Attempting to start on port 6390...${NC}"
        if command -v redis-server &>/dev/null; then
...
start_services() {
    ...
    # Export REDIS_URL to use the new port
    ...
    # 3. Port Selection
    RUNSERVER_PORT=${DJANGO_PORT:-8050}
    if port_free "$RUNSERVER_PORT"; then
        ...
        RUNSERVER_PORT=$(find_available_port 8050 8100)
        if [ -z "$RUNSERVER_PORT" ]; then
            echo -e "${RED}[ERROR] No free ports available in range 8050-8100.${NC}"
            rm -f "$LOCKFILE"
            exit 1
        fi
    ...
    export NGROK_AUTHTOKEN="3EK4AEsl1EbIFf7EL1tjvKovZOS_4cnZrmFpqW3giU8AXtBTQ"
...
PY
    then
        echo -e "Redis Message Broker: \t ${GREEN}[RUNNING]${NC} (Port 6390)"
    else
        echo -e "Redis Message Broker: \t ${RED}[STOPPED]${NC} (Port 6390)"
    fi
